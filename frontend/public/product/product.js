...
window.changePage = function (newPage) {
	...
	$(`#paginationContainer button:nth-of-type(${newPage})`).addClass("current");
};
...
function renderPaginationControls(currentPage, totalPages) {
	...
	if (currentPage > 1) {
		paginationContainer.innerHTML += `
		<i class="fa-solid fa-chevron-left" onclick="changePage(${
			currentPage - 1
		})"></i>`;
	}
	...
	for (let i = 1; i <= totalPages; i++) {
		paginationContainer.innerHTML += `<button class="pagination_button" onclick="changePage(${i})">${i}</button>`;
	}

	// 다음 버튼을 렌더링합니다.
	if (currentPage < totalPages) {
		paginationContainer.innerHTML += `	<i class="fa-solid fa-chevron-right" onclick="changePage(${
			currentPage + 1
		})"></i>`;
	}
}
